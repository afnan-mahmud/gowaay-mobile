/**
 * Admin Rooms Screen — Glassmorphism design, Room management (list, approve, reject)
 */
...
export default function AdminRoomsScreen({ navigation }: any) {
  ...
  const handleApprove = async (id: string) => {
    setActionId(id);
    try {
      const response = await api.admin.approveRoom(id, {});
      if (response.success) {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setActionId(id);
            try {
              const response = await api.admin.rejectRoom(id, {});
              if (response.success) {
    ...
    return (
      <View style={S.card}>
        ...
        <View style={S.roomContent}>
          ...
          <Text style={S.roomPrice}>৳{(item.basePriceTk || 0).toLocaleString()} / night</Text>
  ...
  return (
    <View style={S.root}>
      <FlatList
        ...
        contentContainerStyle={rooms.length === 0 ? S.listEmpty : S.list}
        refreshControl={
